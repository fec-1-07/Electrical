...
(function ($) {
  ...
  $.colorPick = function (element, options) {
    ...
    this.color = this.options.initialColor.toString().toUpperCase();
  ...
  $.colorPick.prototype = {
    ...
    appendToStorage: function (color) {
      const self = this;
      if (this.options.allowRecent === true) {
    ...
    show: function (element, left, top) {
      ...
      $(element).prepend('<div class="colorPickWrapper"><div id="colorPick" style="display:none;top:' + top + 'px;left:' + left + 'px"><span>' + self.options.paletteLabel + '</span></div></div>');

      jQuery.each(self.palette, function (index, item) {
        if (self.options.sizes === true) {
          $("#colorPick").append('<div class="colorPickButton" hexValue="' + item + '" style="background:'+ self.options.textColor+'; color:'+ self.options.textColor+'; height:' + item + 'px;">' + item + '</div>');
        } else {
          const colors = item.split("|");
          if (colors.length > 1) {
            const bg = `linear-gradient(to right, ${colors[0]} 50%, ${colors[1]} 0%)`;
            $("#colorPick").append('<div class="colorPickButton" hexValue="' + item + '" style="background:' + bg + '"></div>');
          } else {
      ...
      if (self.options.allowCustomColor === true) {
        $("#colorPick").append('<input type="text" style="margin-top:5px" class="customColorHash" />');
      }
      if (self.options.allowRecent === true) {
        $("#colorPick").append('<span style="margin-top:5px">' + self.options.recentLabel + '</span>');
